...
---
### まとめ
**階層化されたスタック**では、複数のスタックで繰り返し使用できるリソースを作成できます。全てのスタックには、**エクスポート**名が必要です。この機能を利用することで、役割と責任を分けることができます。例えば、ネットワークチームは、承認された VPC デザインをテンプレートとして作成して提供できます。その後、必要に応じて、VPCスタックのエクスポートを参照するだけです。同様に、セキュリティチームは IAM ロールや EC2 セキュリティグループについても同じ操作を行うことができます。
